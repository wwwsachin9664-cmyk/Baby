...
async def _build_history_view(
    ctx: commands.Context["CricStarBot"], queryset: "QuerySet[Trade]", title: str, admin_url_path: str | None = None
):
    ...
    async def callback(interaction: discord.Interaction["CricStarBot"]):
...
@checks.has_permissions("bd_models.view_trade", "bd_models.view_tradeobject")
async def history_user(ctx: commands.Context["CricStarBot"], user: discord.User, *, flags: UserTradeHistoryFlags):
    """
    ...
    if flags.cricketer:
        queryset = queryset.filter(Q(tradeobjects__ballinstance__ball=flags.cricketer)).distinct()
    if flags.special:
...
@history.command(name="cricketer")
@checks.has_permissions("bd_models.view_trade", "bd_models.view_tradeobject")
async def history_ball(ctx: commands.Context["CricStarBot"], cricketer_id: str, *, flags: TradeHistoryFlags):
    """
    Show the trade history of a cricketer.

    Parameters
    ----------
    cricketer_id: str
        The ID of the cricketer you want to check the history of.
    """

    try:
        ball = await BallInstance.objects.aget(id=int(cricketer_id, 16))
    except ValueError:
...
@checks.has_permissions("bd_models.view_trade", "bd_models.view_tradeobject")
async def trade_info(ctx: commands.Context["CricStarBot"], trade_id: str):
    """
    ...
    from cricstar.packages.trade.history import HistoryView
