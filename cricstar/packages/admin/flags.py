...
from cricstar.core.utils.transformers import BallTransform, SpecialTransform
...
class RarityFlags(FlagConverter):
    chunked: bool = flag(default=True, description="Group together cricketers with the same rarity.")
    include_disabled: bool = flag(
        default=False, description="Include the cricketers that are disabled or with a rarity of 0."
    )


class SpawnFlags(FlagConverter):
    cricketer: BallTransform | None = flag(
        description="The cricketer you want to spawn. Random according to rarities if not specified."
    )
    channel: discord.TextChannel | None = flag(
        description="The channel you want to spawn the cricketer in. Current channel if not specified.", default=None
    )
    n: Range[int, 1, 100] = flag(
        description="The number of cricketers to spawn. If no cricketer was specified, it's random every time.",
        default=1,
    )
    special: SpecialTransform | None = flag(
        description="Force the cricketer to have a special attribute when caught."
    )
    atk_bonus: int | None = flag(description="Force the cricketer to have a specific attack bonus when caught.")
    hp_bonus: int | None = flag(description="Force the cricketer to have a specific health bonus when caught.")


class GiveBallFlags(FlagConverter):
    cricketer: BallTransform = flag(positional=True, description="The cricketer you want to give")
...
class BallsCountFlags(FlagConverter):
    user: discord.User | None = flag(description="The player whose cricketers you are counting")
    cricketer: BallTransform | None = flag(description="Restrict countring to a specific cricketer")
    special: SpecialTransform | None = flag(description="Restrict counting to a special event")
    deleted: bool = flag(default=False, description="Count the deleted cricketers too")
...
class UserTradeHistoryFlags(TradeHistoryFlags):
    cricketer: BallTransform | None = flag(description="The cricketer you want to filter the history by")
