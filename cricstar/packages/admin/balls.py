...
log = logging.getLogger("cricstar.packages.admin.balls")
...
async def _spawn_bomb(
    ...
    cricketer_cls: type["BallSpawnView"],
    cricketer: Ball | None,
    ...
    async def update_message_loop():
        ...
        for i in range(5 * 12 * 10):  # timeout progress after 10 minutes
            await edit_func(
                ...
                f"{settings.collectible_name.title()}: {cricketer or 'Random'}\n"
    ...
    try:
        for i in range(n):
            if not cricketer:
                ball = await cricketer_cls.get_random(ctx.bot)
            else:
                ball = cricketer_cls(ctx.bot, cricketer)
...
@commands.hybrid_group(name=settings.cricstar_slash_name)
async def balls(ctx: commands.Context[CricStarBot]):
    """
...
async def spawn(ctx: commands.Context[CricStarBot], *, flags: SpawnFlags):
    """
    Force spawn a random or specified cricketer.
    """
    ...
    if not cog:
        ...
        # do not replace `cricketers` with `settings.collectible_name`, it is intended
        await ctx.send(
            "The `cricketers` package is not loaded, this command is unavailable.\n"
            "Please resolve the errors preventing this package from loading. Use "
            f'"{prefix}reload cricketers" to try reloading it.',
            ephemeral=True,
        )
        return
    ...
    if flags.n > 1:
        ...
        log.info(
            ...
            f" {flags.cricketer or 'random'} {flags.n} times in {flags.channel or ctx.channel}"
    ...
    if not flags.cricketer:
        ball = await cog.cricketer_cls.get_random(ctx.bot)
    else:
        ball = cog.cricketer_cls(ctx.bot, flags.cricketer)
...
@balls.command()
@checks.has_permissions("bd_models.add_ballinstance")
async def give(ctx: commands.Context[CricStarBot], user: discord.User, *, flags: GiveBallFlags):
    """
    Give the specified cricketer to a player.
    ...
    log.info(
        f"{ctx.author} gave {settings.collectible_name} {flags.cricketer.country} (`{instance.pk:0X}`) "
...
async def balls_info(ctx: commands.Context[CricStarBot], cricketer_id: str):
    """
    Show information about a cricketer.

    Parameters
    ----------
    cricketer_id: str
        The ID of the cricketer you want to get information about.
    """
    try:
        pk = int(cricketer_id, 16)
    except ValueError:
...
async def balls_delete(ctx: commands.Context[CricStarBot], cricketer_id: str, soft_delete: bool = True):
    """
    Delete a cricketer.
    ...
    ----------
    cricketer_id: str
        The ID of the cricketer you want to delete.
    soft_delete: bool
        Whether the cricketer should be kept in database or fully wiped.
    """
    try:
        ballIdConverted = int(cricketer_id, 16)
    except ValueError:
        await ctx.send(f"The {settings.collectible_name} ID you gave is not valid.", ephemeral=True)
        return
    try:
    ...
    if soft_delete:
        ball.deleted = True
        await ball.asave()
        await ctx.send(f"{settings.collectible_name.title()} {cricketer_id} soft deleted.", ephemeral=True)
        log.info(f"{ctx.author} soft deleted {ball}({ball.pk}).", extra={"webhook": True})
    else:
        await ball.adelete()
        await ctx.send(f"{settings.collectible_name.title()} {cricketer_id} hard deleted.", ephemeral=True)
...
async def balls_transfer(ctx: commands.Context[CricStarBot], cricketer_id: str, user: discord.User):
    """
    Transfer a cricketer to another user.

    Parameters
    ----------
    cricketer_id: str
        The ID of the cricketer you want to transfer.
    user: discord.User
        The user you want to transfer the cricketer to.
    """
    try:
        ballIdConverted = int(cricketer_id, 16)
    except ValueError:
...
async def balls_reset(
    ctx: commands.Context[CricStarBot], user: discord.User, percentage: int | None = None, soft_delete: bool = True
):
    """
    Reset a player's cricketers.

    Parameters
    ----------
    user: discord.User
        The user you want to reset the cricketers of.
    percentage: int | None
        The percentage of cricketers to delete, if not all. Used for sanctions.
    soft_delete: bool
        If true, the cricketers will be marked as deleted instead of being removed from the
        database.
    """
...
@checks.has_permissions("bd_models.view_ballinstance")
async def balls_count(ctx: commands.Context[CricStarBot], *, flags: BallsCountFlags):
    """
    Count the number of cricketers that a player has or how many exist in total.
    """
    filters = {}
    if flags.cricketer:
        filters["ball"] = flags.cricketer
    if flags.special:
    ...
    country = f"{flags.cricketer.country} " if flags.cricketer else ""
