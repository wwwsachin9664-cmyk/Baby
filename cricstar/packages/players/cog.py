...
from cricstar.core.utils.enums import TRADE_COOLDOWN_POLICY_MAP as TRADE_POLICY_MAP
from cricstar.core.utils.menus import ItemFormatter, ListSource, Menu, dynamic_chunks
...
class Player(commands.GroupCog):
    ...
    async def psettings(self, interaction: discord.Interaction["CricStarBot"]):
        """
    ...
    async def friend_add(self, interaction: discord.Interaction["CricStarBot"], user: discord.User):
        """
    ...
    async def friend_remove(self, interaction: discord.Interaction["CricStarBot"], user: discord.User):
        """
        Remove a friend.
    ...
    async def friend_list(self, interaction: discord.Interaction["CricStarBot"]):
        """
    ...
    async def block_add(self, interaction: discord.Interaction["CricStarBot"], user: discord.User):
        """
        Block another user.
    ...
    async def block_remove(self, interaction: discord.Interaction["CricStarBot"], user: discord.User):
        """
        Unblock a user.
    ...
    async def blocked_list(self, interaction: discord.Interaction["CricStarBot"]):
        """
    ...
    async def info(self, interaction: discord.Interaction["CricStarBot"]):
        """
        ...
        total_cricketers = len(bot_cricketers)
        owned_cricketers = set(
            [x[0] async for x in player.balls.filter(ball__enabled=True).distinct().values_list("ball_id")]
        )

        if total_cricketers > 0:
            completion_percentage = f"{round(len(owned_cricketers) / total_cricketers * 100, 1)}%"
        else:
