...
from cricstar.core.utils.menus import Menu, ModelSource
...
class Trade(commands.GroupCog):
    ...
    async def add(self, interaction: Interaction, cricketer: BallInstanceTransform):
        """
        Add a cricketer to your trade proposal. You must have a trade open.

        Parameters
        ----------
        cricketer: BallInstance
            The cricketer you are adding to your trade.
        """
        ...
        trade, trader = result
        try:
            await trader.add_to_proposal(BallInstance.objects.filter(id=cricketer.pk))
    ...
    async def remove(self, interaction: Interaction, cricketer: BallInstanceTransform):
        """
        Remove a cricketer from your trade proposal. You must have a trade open.

        Parameters
        ----------
        cricketer: BallInstance
            The cricketer you are removing from your trade.
        """
        result = await self.get_trade(interaction)
        if result is None:
        ...
        trade, trader = result
        try:
            await trader.remove_from_proposal(BallInstance.objects.filter(id=cricketer.pk))
        except TradeError as e:
            await interaction.response.send_message(e.error_message, ephemeral=True)
        else:
            await trade.edit_message(None)
    ...
    async def history(
        ...
        cricketer: Ball | None
            The cricketer you want to filter the trade history by.
        ...
        if cricketer and special:
            queryset = queryset.filter(
                Q(tradeobject__ballinstance__ball=cricketer) & Q(tradeobject__ballinstance__special=special)
            ).distinct()
        elif cricketer:
            queryset = queryset.filter(Q(tradeobject__ballinstance__ball=cricketer)).distinct()
        elif special:
    ...
    async def bulk_add(
        ...
        """
        Bulk add cricketers to the ongoing trade, with paramaters to aid with searching.

        Parameters
        ----------
        cricketer: Ball
            The cricketer you would like to filter the results to
        sort: SortingChoices
