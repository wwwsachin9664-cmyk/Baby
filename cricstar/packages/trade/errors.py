...
log = logging.getLogger("cricstar.packages.trade")
...
class NotTradeableError(TradeError):
    """
    The cricketer is not tradeable (ball, ballinstance or special)
    """
...
class AlreadyLockedError(TradeError):
    """
    Raised when a locked cricketer is about to be traded.
    """
...
class NotProposedError(TradeError):
    """
    A cricketer was attempted to be removed when it was not part of the proposal.
    """
...
class OwnershipError(TradeError):
    """
    A cricketer is attempting to be traded, but it's not owned by the player.
    """
...
class IntegrityError(TradeError):
    ...
    This happens when the ownership of the cricketer changes while processing the trade.
    """
