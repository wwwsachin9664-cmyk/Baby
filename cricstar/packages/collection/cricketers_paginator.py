...
class CricketersViewer(LayoutView):
    header = TextDisplay("")
    select_row = ActionRow()

    @select_row.select()
    async def selected(self, interaction: discord.Interaction["CricStarBot"], select: Select):
...
class CricketersDuplicateSource(LayoutView):
    ...
    @select_row.select()
    async def callback(self, interaction: discord.Interaction["CricStarBot"], select: Select):
        ...
        if self.is_special:
            ...
            cricketer = await Ball.objects.aget(id=select.values[0])
            name = cricketer.country
            balls_query = balls_query.filter(ball=cricketer).annotate(
                specials=Count(
                    "id",
            ...
            grouped_query = (
                BallInstance.objects.filter(player=player, ball=cricketer)
        ...
        if cricketer:
            emoji = interaction.client.get_emoji(cricketer.emoji_id)
            if emoji:
