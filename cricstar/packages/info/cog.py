...
from cricstar import __version__ as cricstar_version
from cricstar.core.utils.django import row_count_estimate
from cricstar.core.utils.formatting import pagify
from bd_models.models import Ball
from bd_models.models import balls as cricketers
...
log = logging.getLogger("cricstar.packages.info")
...
class Info(commands.Cog):
    ...
    async def _get_10_balls_emojis(self) -> list[discord.Emoji]:
        balls: list[Ball] = random.choices(
            [x for x in cricketers.values() if x.enabled], k=min(10, len(cricketers))
        )
    ...
    async def about(self, interaction: discord.Interaction["CricStarBot"]):
        """
        ...
        balls_count = len([x for x in cricketers.values() if x.enabled])
        ...
        embed.description = (
            ...
            f"*Running version **[{cricstar_version}]({settings.repository}/releases)***\n"
    ...
    @app_commands.command()
    async def help(self, interaction: discord.Interaction["CricStarBot"]):
        """
