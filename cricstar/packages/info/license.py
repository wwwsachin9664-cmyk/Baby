...
from cricstar.core.utils.menus import ChunkedListSource, Menu, SelectFormatter
...
class ExtraLicenseView(LayoutView):
    ...
    @row.select()
    async def extra_package_select(self, interaction: discord.Interaction["CricStarBot"], select: Select):
...
class LicenseInfo(View):
    ...
    async def license_info(self, interaction: discord.Interaction["CricStarBot"], _: Button):
        await interaction.response.send_message(
            "This bot is an instance of CricStar-DiscordBot "
            ...
            ephemeral=True,
            file=discord.File(
                get_license_files(importlib.metadata.distribution("cricstar"))[0].locate(), filename="LICENSE.txt"
            ),
        )

    @button(label="3rd party packages")
    async def extra_packages(self, interaction: discord.Interaction["CricStarBot"], _: Button):
