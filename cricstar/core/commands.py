...
from cricstar.core.dev import send_interactive
from cricstar.core.discord import LayoutView, View
from cricstar.core.utils.formatting import pagify
from cricstar.core.utils.menus import Menu, TextFormatter, TextSource
from bd_models.models import Ball
from settings.models import load_settings, settings

log = logging.getLogger("cricstar.core.commands")

if TYPE_CHECKING:
    from admin_panel.logging import DequeHandler

    from .bot import CricStarBot
...
class SimpleCheckView(View):
    ...
    async def confirm_button(self, interaction: discord.Interaction["CricStarBot"], button: discord.ui.Button):
...
class Core(commands.Cog):
    ...
    async def reload_package(self, package: str, *, with_prefix=False):
        try:
            ...
            if not with_prefix:
                await self.reload_package("cricstar.packages." + package, with_prefix=True)
                return
            raise
    ...
    async def migrateemotes(self, ctx: commands.Context):
        ...
        The emoji IDs of the cricketers are updated afterwards.
