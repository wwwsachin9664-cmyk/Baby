...
from cricstar.core.commands import Core
...
log = logging.getLogger("cricstar.core.bot")
http_counter = Histogram("discord_http_requests", "HTTP requests", ["key", "code"])
impersonations: dict[int, discord.Member] = {}

DEFAULT_PACKAGES = (
    ("admin", "cricstar.packages.admin"),
    ("collection", "cricstar.packages.collection"),
    ("guildconfig", "cricstar.packages.guildconfig"),
    ("cricketers", "cricstar.packages.cricketers"),
    ("info", "cricstar.packages.info"),
    ...
    ("legendarypack", "cricstar.packages.legendarypack"),
    ("rankup", "cricstar.packages.rankup"),
)


def owner_check(ctx: commands.Context[CricStarBot]):
...
class Translator(app_commands.Translator):
    async def translate(self, string: locale_str, locale: Locale, context: TranslationContextTypes) -> str | None:
        text = (
            string.message.replace("cricketers", settings.plural_collectible_name)
            .replace("cricketer", settings.collectible_name)
            .replace("/balls", f"/{settings.cricstar_slash_name}")
            .replace("CricStar", settings.bot_name)
        )
...
class CommandTree[Bot: CricStarBot](app_commands.CommandTree[Bot]):
...
class CricStarBot(commands.AutoShardedBot):
    """
    CricStar Discord bot
    """

    def __init__(
        self,
        command_prefix: PrefixType[CricStarBot],
