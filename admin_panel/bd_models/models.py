...
def transform_media(path: str) -> str:
    return path.replace("/static/uploads/", "").replace("/cricstar/core/image_generator/src/", "default/")
...
class GuildConfig(models.Model):
    ...
    enabled = models.BooleanField(help_text="Whether the bot will spawn cricketers in this guild", default=True)
...
class Ball(models.Model):
    ...
    capacity_name = models.CharField(max_length=64, help_text="Name of the cricketer's capacity")
    capacity_description = models.CharField(max_length=256, help_text="Description of the cricketer's capacity")
...
class BallInstance(models.Model):
    ...
    def short_description(self, *, is_trade: bool = False) -> str:
        ...
        return f"{text}#{self.pk:0X} {self.cricketer.country}"
    ...
    def is_tradeable(self) -> bool:
        return self.tradeable and self.cricketer.tradeable and getattr(self.specialcard, "tradeable", True)

    @property
    def attack(self) -> int:
        bonus = int(self.cricketer.attack * self.attack_bonus * 0.01)
        return self.cricketer.attack + bonus

    @property
    def health(self) -> int:
        bonus = int(self.cricketer.health * self.health_bonus * 0.01)
    ...
    def description(
        ...
        *,
        ...
        if include_emoji:
            ...
            if isinstance(self.cricketer, Ball):
    ...
    async def prepare_for_message(
        self, interaction: discord.Interaction["CricStarBot"]
