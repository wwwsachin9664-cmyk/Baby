...
from cricstar import __version__ as bot_version
...
log = logging.getLogger("cricstar")
...
def patch_gateway(proxy_url: str):
    ...
    CricStarBot.is_ws_ratelimited = is_ws_ratelimited
    CricStarBot.before_identify_hook = before_identify_hook
...
async def shutdown_handler(bot: CricStarBot, signal_type: str | None = None):
    if signal_type:
...
def global_exception_handler(bot: CricStarBot, loop: asyncio.AbstractEventLoop, context: dict):
    """
...
def bot_exception_handler(bot: CricStarBot, bot_task: asyncio.Future):
    """
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options: Unpack[CLIFlags]):
        ...
        try:
            ...
            db_url = os.environ.get("CRICSTARBOT_DB_URL", os.environ.get("DATABASE_URL", None))
            if not db_url:
